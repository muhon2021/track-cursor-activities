...
DROP POLICY IF EXISTS "Authenticated users can view sources" ON public.knowledge_sources;
...
DROP POLICY IF EXISTS "Users can view own sources" ON public.user_knowledge_sources;
...
DROP POLICY IF EXISTS "Users can insert own sources" ON public.user_knowledge_sources;
...
DROP POLICY IF EXISTS "Users can update own sources" ON public.user_knowledge_sources;
...
DROP POLICY IF EXISTS "Users can delete own sources" ON public.user_knowledge_sources;
...
DROP POLICY IF EXISTS "Admins can view all user sources" ON public.user_knowledge_sources;
...
-- Add foreign key to user_knowledge_files if table exists (created in 20260101000007)
DO $$
BEGIN
  IF EXISTS (
