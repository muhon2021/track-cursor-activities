...
DROP POLICY IF EXISTS "Users can view their own personalizations" ON public.user_agent_personalizations;
CREATE POLICY "Users can view their own personalizations"
  ON public.user_agent_personalizations FOR SELECT
  TO authenticated
  USING (user_id = auth.uid());

DROP POLICY IF EXISTS "Users can insert their own personalizations" ON public.user_agent_personalizations;
CREATE POLICY "Users can insert their own personalizations"
  ON public.user_agent_personalizations FOR INSERT
  TO authenticated
  WITH CHECK (user_id = auth.uid());

DROP POLICY IF EXISTS "Users can update their own personalizations" ON public.user_agent_personalizations;
...
DROP POLICY IF EXISTS "Users can delete their own personalizations" ON public.user_agent_personalizations;
