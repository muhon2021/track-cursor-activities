...
DROP POLICY IF EXISTS "Authenticated users can read task_streams" ON task_streams;
CREATE POLICY "Authenticated users can read task_streams"
  ON task_streams FOR SELECT
  USING (auth.uid() IS NOT NULL);

DROP POLICY IF EXISTS "Authenticated users can create task_streams" ON task_streams;
CREATE POLICY "Authenticated users can create task_streams"
  ON task_streams FOR INSERT
  WITH CHECK (auth.uid() IS NOT NULL);

DROP POLICY IF EXISTS "Stream creators and admins can update task_streams" ON task_streams;
...
DROP POLICY IF EXISTS "Members can read their stream memberships" ON task_stream_members;
CREATE POLICY "Members can read their stream memberships"
  ON task_stream_members FOR SELECT
  USING (auth.uid() IS NOT NULL);

DROP POLICY IF EXISTS "Stream owners and admins can manage members" ON task_stream_members;
...
DROP POLICY IF EXISTS "Anyone authenticated can read task_categories" ON task_categories;
CREATE POLICY "Anyone authenticated can read task_categories"
  ON task_categories FOR SELECT
  USING (auth.uid() IS NOT NULL);

DROP POLICY IF EXISTS "Admins can manage task_categories" ON task_categories;
...
DROP POLICY IF EXISTS "Authenticated users can read task_attachments" ON task_attachments;
CREATE POLICY "Authenticated users can read task_attachments"
  ON task_attachments FOR SELECT
  USING (auth.uid() IS NOT NULL);

DROP POLICY IF EXISTS "Authenticated users can upload task_attachments" ON task_attachments;
CREATE POLICY "Authenticated users can upload task_attachments"
  ON task_attachments FOR INSERT
  WITH CHECK (auth.uid() IS NOT NULL);

DROP POLICY IF EXISTS "Uploaders and admins can delete task_attachments" ON task_attachments;
...
DROP POLICY IF EXISTS "Authenticated users can read task_contributors" ON task_contributors;
CREATE POLICY "Authenticated users can read task_contributors"
  ON task_contributors FOR SELECT
  USING (auth.uid() IS NOT NULL);

DROP POLICY IF EXISTS "Task assignees and admins can manage task_contributors" ON task_contributors;
