...
DROP POLICY IF EXISTS "Anyone can read app_modules" ON app_modules;
CREATE POLICY "Anyone can read app_modules"
  ON app_modules FOR SELECT
  USING (true);

-- Only admins can update modules
DROP POLICY IF EXISTS "Admins can update app_modules" ON app_modules;
...
DROP POLICY IF EXISTS "Admins can insert app_modules" ON app_modules;
...
DROP POLICY IF EXISTS "Users can read own module permissions" ON user_module_permissions;
...
DROP POLICY IF EXISTS "Admins can read all module permissions" ON user_module_permissions;
