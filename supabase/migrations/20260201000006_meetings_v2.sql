...
BEGIN
  IF EXISTS (
    ...
    WHERE table_schema = 'public' AND table_name = 'meeting_categorizations' AND column_name = 'meeting_id'
  ) THEN
    CREATE INDEX IF NOT EXISTS idx_categorizations_meeting ON meeting_categorizations(meeting_id);
  END IF;
  IF EXISTS (
    SELECT 1 FROM information_schema.columns
    WHERE table_schema = 'public' AND table_name = 'meeting_categorizations' AND column_name = 'category'
  ) THEN
...
DROP POLICY IF EXISTS "Authenticated users can view series" ON meeting_series;
CREATE POLICY "Authenticated users can view series" ON meeting_series
  FOR SELECT TO authenticated USING (true);
DROP POLICY IF EXISTS "Users can manage their own series" ON meeting_series;
...
CREATE POLICY "Authenticated users can view agenda items" ON meeting_agenda_items
  FOR SELECT TO authenticated USING (true);
DROP POLICY IF EXISTS "Authenticated users can manage agenda items" ON meeting_agenda_items;
...
DROP POLICY IF EXISTS "Authenticated users can view takeaways" ON meeting_takeaways;
CREATE POLICY "Authenticated users can view takeaways" ON meeting_takeaways
  FOR SELECT TO authenticated USING (true);
DROP POLICY IF EXISTS "Authenticated users can manage takeaways" ON meeting_takeaways;
CREATE POLICY "Authenticated users can manage takeaways" ON meeting_takeaways
  FOR ALL TO authenticated USING (true) WITH CHECK (true);

-- Participants
ALTER TABLE meeting_participants ENABLE ROW LEVEL SECURITY;
DROP POLICY IF EXISTS "Authenticated users can view participants" ON meeting_participants;
CREATE POLICY "Authenticated users can view participants" ON meeting_participants
  FOR SELECT TO authenticated USING (true);
DROP POLICY IF EXISTS "Authenticated users can manage participants" ON meeting_participants;
CREATE POLICY "Authenticated users can manage participants" ON meeting_participants
  FOR ALL TO authenticated USING (true) WITH CHECK (true);

-- Transcripts
ALTER TABLE meeting_transcripts ENABLE ROW LEVEL SECURITY;
DROP POLICY IF EXISTS "Authenticated users can view transcripts" ON meeting_transcripts;
CREATE POLICY "Authenticated users can view transcripts" ON meeting_transcripts
  FOR SELECT TO authenticated USING (true);
DROP POLICY IF EXISTS "Authenticated users can manage transcripts" ON meeting_transcripts;
...
DROP POLICY IF EXISTS "Authenticated users can view assignments" ON meeting_assignments;
CREATE POLICY "Authenticated users can view assignments" ON meeting_assignments
  FOR SELECT TO authenticated USING (true);
DROP POLICY IF EXISTS "Authenticated users can manage assignments" ON meeting_assignments;
