...
-- kb_source_config
DROP POLICY IF EXISTS "Admins manage kb_source_config" ON public.kb_source_config;
CREATE POLICY "Admins manage kb_source_config"
  ON public.kb_source_config FOR ALL TO authenticated
  USING (public.has_role(auth.uid(), 'admin'))
  WITH CHECK (public.has_role(auth.uid(), 'admin'));

DROP POLICY IF EXISTS "Authenticated read kb_source_config" ON public.kb_source_config;
CREATE POLICY "Authenticated read kb_source_config"
  ON public.kb_source_config FOR SELECT TO authenticated
  USING (true);

-- kb_eval_*
DROP POLICY IF EXISTS "Admins manage kb_eval_runs" ON public.kb_eval_runs;
CREATE POLICY "Admins manage kb_eval_runs"
  ON public.kb_eval_runs FOR ALL TO authenticated
  USING (public.has_role(auth.uid(), 'admin'))
  WITH CHECK (public.has_role(auth.uid(), 'admin'));

DROP POLICY IF EXISTS "Admins manage kb_eval_results" ON public.kb_eval_results;
CREATE POLICY "Admins manage kb_eval_results"
  ON public.kb_eval_results FOR ALL TO authenticated
  USING (public.has_role(auth.uid(), 'admin'))
  WITH CHECK (public.has_role(auth.uid(), 'admin'));

DROP POLICY IF EXISTS "Admins manage kb_eval_test_cases" ON public.kb_eval_test_cases;
...
-- kb_reembed_*
DROP POLICY IF EXISTS "Admins manage kb_reembed_jobs" ON public.kb_reembed_jobs;
CREATE POLICY "Admins manage kb_reembed_jobs"
  ON public.kb_reembed_jobs FOR ALL TO authenticated
  USING (public.has_role(auth.uid(), 'admin'))
  WITH CHECK (public.has_role(auth.uid(), 'admin'));

DROP POLICY IF EXISTS "Admins manage kb_reembed_job_items" ON public.kb_reembed_job_items;
...
DROP POLICY IF EXISTS "Admins manage kb_source_permissions" ON public.kb_source_permissions;
CREATE POLICY "Admins manage kb_source_permissions"
  ON public.kb_source_permissions FOR ALL TO authenticated
  USING (public.has_role(auth.uid(), 'admin'))
  WITH CHECK (public.has_role(auth.uid(), 'admin'));

DROP POLICY IF EXISTS "Authenticated read kb_source_permissions" ON public.kb_source_permissions;
...
DROP POLICY IF EXISTS "Admins manage knowledge_sources" ON public.knowledge_sources;
...
DROP POLICY IF EXISTS "Admins manage embedding_queue" ON public.embedding_queue;
