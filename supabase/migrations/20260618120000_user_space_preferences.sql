...
DROP POLICY IF EXISTS "Users can view own space preferences" ON public.user_space_preferences;
CREATE POLICY "Users can view own space preferences"
  ON public.user_space_preferences FOR SELECT
  USING (auth.uid() = user_id);

DROP POLICY IF EXISTS "Users can insert own space preferences" ON public.user_space_preferences;
CREATE POLICY "Users can insert own space preferences"
  ON public.user_space_preferences FOR INSERT
  WITH CHECK (auth.uid() = user_id);

DROP POLICY IF EXISTS "Users can update own space preferences" ON public.user_space_preferences;
